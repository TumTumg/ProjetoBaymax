...
def createApp():
    ...
    hardwareDAO = HardwareDAO(Config.ARDUINO_PORT)
    ...
    @app.route('/baymax/start', methods=['POST'])
    def startBaymax():
        try:
            baymaxService.main()  # Isso iniciará o BaymaxService
            return jsonify({'status': 'Baymax iniciado'}), 200
        except Exception as e:
            return jsonify({'error': str(e)}), 500
